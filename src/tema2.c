...
int closest_preceding_finger(int key) {
     ...
     ******************************************************/
    for (int i = M - 1; i >= 0; i--) {
        if (in_interval(self.finger[i].node, self.id, key) && self.finger[i].node != self.id) {
...
void handle_lookup_request(LookupMsg *msg) {
    ...
    if (msg->key == self.id) {
        ...
        MPI_Send(msg, sizeof(LookupMsg), MPI_BYTE, rank_from_id(closest_preceding_finger(msg->key)), TAG_LOOKUP_REQ, MPI_COMM_WORLD);
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < nr_lookups; i++) {
        messages[i] = malloc(sizeof(LookupMsg));
        messages[i]->initiator_id = self.id;
        messages[i]->current_id = self.id;
        messages[i]->key = lookups[i];
        messages[i]->path_len = 0;

        MPI_Send(messages[i], sizeof(LookupMsg), MPI_BYTE, world_rank, TAG_LOOKUP_REQ, MPI_COMM_WORLD);
    }
    ...
    if (nr_lookups == 0) {
        ...
        for (int i = 0; i < world_size; i++) {
            if (i != world_rank) {
                MPI_Send(&done_msg, sizeof(LookupMsg), MPI_BYTE, i, TAG_DONE, MPI_COMM_WORLD);
            }
        }
        done_sent = 1;
    }
    ...
    while (1) {
        ...
        if (status.MPI_TAG == TAG_LOOKUP_REQ) {
            ...
            if (lookups_left == 0 && !done_sent){
                ...
                done_sent = 1;
                if (done_total == world_size - 1)
                    break;
            }
        } else if (status.MPI_TAG == TAG_DONE) {
            if (!done[status.MPI_SOURCE]) {
                done[status.MPI_SOURCE] = 1;
                done_total++;
            }

            if (done_total == world_size - 1 && lookups_left == 0) {
                break;
            }
        }
    }
